...
import { NeoCard, NeoButton, NeoInput, NeoTabs, NeoBadge, NeoDialog, AppLogo, NeoSelect } from '../components/NeoUI';
import { User, CategoryState } from '../types';
import { Settings, Save, Trash2, Download, Plus, TrendingDown, TrendingUp, Pencil, Users, UserPlus } from 'lucide-react';
...
export const SettingsPage: React.FC<SettingsPageProps> = ({ 
  ...
  const [newUserForm, setNewUserForm] = useState({ name: '', avatar: '', color: 'bg-brand-orange' });
  ...
  const handleAddExpenseCat = () => {
    if (newExpenseCat && !categories.expense.includes(newExpenseCat)) {
      onUpdateCategories({
        ...categories,
  ...
  const handleAddIncomeCat = () => {
    if (newIncomeCat && !categories.income.includes(newIncomeCat)) {
        onUpdateCategories({
          ...categories,
  ...
  const handleDeleteCat = (cat: string, type: 'EXPENSE' | 'INCOME') => {
    if (type === 'EXPENSE') {
        onUpdateCategories({
            ...categories,
  ...
  const handleInviteUser = (e: React.FormEvent) => {
      ...
      onAddUser({
          ...
          avatar: `https://i.pravatar.cc/150?u=${newUserForm.name}`,
          color: newUserForm.color
      });
      setIsInviteOpen(false);
      setNewUserForm({ name: '', avatar: '', color: 'bg-brand-orange' });
  };
  ...
  return (
    <div className="space-y-8">
                ...
                <NeoCard title={t.settings.profile.user} className="space-y-6">
                    <div className="flex flex-col md:flex-row items-center gap-8 mb-6 justify-center">
                        <div className="relative group">
                            ...
                            />
                            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-black text-white text-[10px] font-bold px-2 py-1 uppercase">
                                Edit URL
                            </div>
                        </div>
              ...
              <div className="space-y-6">
                  <NeoCard title={t.settings.team.title} color="white">
                      <div className="flex justify-between items-center mb-6">
                          ...
                          <NeoButton onClick={() => setIsInviteOpen(true)} icon={<UserPlus />}>{t.settings.team.invite}</NeoButton>
                      </div>
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {users.map(user => (
                              <div key={user.id} className="border-2 border-black p-4 flex items-center gap-4 shadow-neo-sm bg-white">
                                  <img src={user.avatar} alt={user.name} className="w-12 h-12 rounded-full border-2 border-black" />
                                  <div>
                                      <div className="font-black uppercase text-lg">{user.name}</div>
                                      <div className="text-xs font-bold text-gray-500 uppercase">
                                          {user.id === currentUser.id ? 'You (Admin)' : 'Collaborator'}
                                      </div>
                                  </div>
                              </div>
                          ))}
          ...
          {activeTab === 'SYSTEM' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <NeoCard title={t.settings.system.about} className="h-full flex flex-col items-center justify-center text-center">
                      ...
                      <h3 className="font-black text-3xl uppercase tracking-tighter mb-1">TRESO</h3>
      ...
      <NeoDialog
         ...
         title={t.settings.team.addUser}
      >
         <form onSubmit={handleInviteUser} className="space-y-4">
            <div className="space-y-1">
                <label className="font-bold text-sm uppercase">{t.settings.team.newUserName}</label>
                <NeoInput 
            ...
            <div className="space-y-1">
                <label className="font-bold text-sm uppercase">{t.settings.team.color}</label>
                <NeoSelect
                    value={newUserForm.color}
                    onChange={(e) => setNewUserForm({...newUserForm, color: e.target.value})}
                >
                    <option value="bg-brand-orange">Orange</option>
                    <option value="bg-brand-accent">Purple</option>
                    <option value="bg-brand-green">Green</option>
                    <option value="bg-brand-red">Red</option>
                </NeoSelect>
            </div>
             <div className="flex gap-2 pt-2">
                <NeoButton type="submit" className="w-full" icon={<UserPlus />}>{t.settings.team.invite}</NeoButton>
                <NeoButton type="button" variant="ghost" onClick={() => setIsInviteOpen(false)}>{t.transactions.cancel}</NeoButton>
            </div>
         </form>
      </NeoDialog>
    </div>
